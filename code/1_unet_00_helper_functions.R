...
createDataset <- function(data,
  ...
  dataset = dataset %>%
    ...
    dataset_map(~.x %>% list_modify(
      ...
      msk = tf$subtract(.x$msk, tf$constant(value = 1L, dtype = tf$uint8, shape = c(tileSize, tileSize, 1L))) %>%
        tf$one_hot(depth = noSpec, dtype = tf$uint8) %>% # one-hot encode masks
    ...
    dataset_map(~.x %>% list_modify( # set shape to avoid error at fitting stage "tensor of unknown rank"
      ...
      msk = tf$reshape(.x$msk, shape = c(tileSize, tileSize, noSpec))
    ))
