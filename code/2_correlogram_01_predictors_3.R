...
# coresp = data.frame(dist = coresp$mean.of.class, correlation = coresp$correlation)[coresp$n > 2000,]
# cotest = data.frame(dist = cotest$mean.of.class, correlation = cotest$correlation)[cotest$n > 2000,]
# 
# 
...
testzoo = rollmean(cotest$correlation[1:length(cotest$correlation)], k=11, na.pad = T, fill = NA)
testroll = roll_mean(cotest$correlation[1:length(cotest$correlation)], width = 10, complete_obs = F) #, min_obs = 1000)
testrollapply = rollapply(cotest$correlation[1:length(cotest$correlation)], width = 10, FUN = mean, na.rm=T, partial=TRUE, align = "right", fill = NA)


length(testzoo)
length(testroll)
length(testrollapply)

plot(1:100,testzoo[1:100], ylim=c(0,0.8))
points(1:100,testroll[1:100], col = "red")
points(1:100,testrollapply[1:100], col="green")

cotest$correlation = range01(cotest$correlation)
cotest_rmean = testrollapply = rollapply(cotest$correlation[1:length(cotest$correlation)], width = 10000, FUN = mean, na.rm=T, partial=TRUE, align = "right", fill = NA)
cotest_rsd = testrollapply = rollapply(cotest$correlation[1:length(cotest$correlation)], width = 1000, FUN = sd, na.rm=T, partial=TRUE, align = "right", fill = NA)


coresp$correlation = range01(coresp$correlation)
coresp_rmean = testrollapply = rollapply(coresp$correlation[1:length(coresp$correlation)], width = 10000, FUN = mean, na.rm=T, partial=TRUE, align = "right", fill = NA)
coresp_rsd = testrollapply = rollapply(coresp$correlation[1:length(coresp$correlation)], width = 1000, FUN = sd, na.rm=T, partial=TRUE, align = "right", fill = NA)
...
polygon(c(dftest$x[1:100],rev(dftest$x[1:100])),c(dftest$L[1:100],rev(dftest$U[1:100])),col = col_testp, border = FALSE)
